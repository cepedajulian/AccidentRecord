...
<div id="content" class="col-xs-12 col-sm-10">
	...
	<?php echo form_open('config/searchLocalidades', array('id' => 'formLocalidades', 'class'=> 'form-horizontal bootstrap-validator-form')); ?>
	...
	<div class="row">
		<div class="col-xs-12">
			...
			<?php if (!empty($this->session->flashdata('total'))){?>
				<?php echo "<b>Total de registros procesados:</b> ". $this->session->flashdata('total');?>
				<?php echo "<br><b>Total de rutas nuevas:</b> ". $this->session->flashdata('nuevas');?>
				<?php echo "<br><b>Total de rutas repetidas:</b> ". $this->session->flashdata('repetidas')."<br>";?>
			<?php }?>
			
			<br>
			
			<?php if (!empty($permisos['config_clocalidades'])){?>
				<a href="<?php echo site_url('/config/clocalidades/') ?>">
			...
			<?php if (!empty($permisos['config_importlocalidades'])){?>
				<a href="<?php echo site_url('/config/importlocalidades/') ?>">
					<button class="btn btn-primary btn-label-left" type="button">
						<span><i class="fa fa-plus-circle"></i></span>
			...
			<div class="box">
				<div class="box-header">
					<div class="box-name">
						...
						<span>Registro de localidades (<?php echo count($localidades); ?>)</span>
					</div>
					<div class="box-icons">
						...
						<tbody>
							...
							foreach($localidades as $localidad) 
							{
								echo "<tr>";
								    ...
								    echo "<td>".strtoupper($localidad->localidad)."</td>";
								    echo "<td width='120px' align='right'>";
									    if (!empty($permisos['config_ulocalidades'])){
									    	echo "<a href='".site_url('/config/ulocalidades/'.$localidad->id)."' title='Edicion' alt='Edicion'>";
												echo "<i class='fa fa-lg  fa-pencil-square-o'></i></span>";
											echo "</a>&nbsp;&nbsp;";
								    	}
								    	
										if (!empty($permisos['config_dlocalidades'])){
											echo "<a href='".site_url('/config/dlocalidades/'.$localidad->id)."' title='Borrar' alt='Borrar'>";
